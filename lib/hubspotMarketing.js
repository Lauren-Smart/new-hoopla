...
async function fetchMarketingPipeline() {
  // TEMPORARY: not catching here on purpose so the real error surfaces via
  // GET /api/marketing-sync's top-level catch instead of being swallowed as
  // null. Remove this comment and restore the try/catch (see git history)
  // once the $0/null pipeline bug is diagnosed.
